...
func init() {
	srvfactory.RegisterServiceInitializer("hex", create)
}

func create(args services.InitArgs) error {
	var typedConf Conf
	if err := json.Unmarshal(args.RawConf, &typedConf); err != nil {
		return fmt.Errorf("failed to initialize service %d (hex): %w", args.SID, err)
	}
	if err := typedConf.ValidateAndDefaults("hex", args.IsStreamingMode); err != nil {
		return fmt.Errorf("failed to initialize service %d (hex): %w", args.SID, err)
	}
	...
	args.APIRoutes.Any(
		fmt.Sprintf("/service/%d/hex/*path", args.SID),
		hexActions.AnyPath,
	)
	log.Info().Int("args.SID", args.SID).Msg("Proxy for Hex enabled")
	return nil
}
