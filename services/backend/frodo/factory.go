...
func init() {
	srvfactory.RegisterServiceInitializer("frodo", create)
}

func create(args services.InitArgs) error {
	var typedConf Conf
	if err := json.Unmarshal(args.RawConf, &typedConf); err != nil {
		return fmt.Errorf("failed to initialize service %d (frodo): %w", args.SID, err)
	}
	if typedConf.SessionValType == "" {
		typedConf.SessionValType = session.SessionTypeNone
	}
	if err := typedConf.Validate("frodo", args.IsStreamingMode); err != nil {
		return fmt.Errorf("failed to initialize service %d (frodo): %w", args.SID, err)
	}
	...
	if len(typedConf.Limits) > 0 {
		frodoReqCounter = args.Alarm.Register(
			fmt.Sprintf("%d/frodo", args.SID),
			typedConf.Alarm,
			typedConf.Limits,
		)
	}
	...
	case guard.GuardTypeToken:
		grd = token.NewGuard(
			args.Ctx,
			fmt.Sprintf("/service/%d/frodo", args.SID),
	...
	frodoActions, err := NewFrodoProxy(
		args.Ctx,
		&typedConf.ProxyConf,
		&cnc.EnvironConf{
			CNCAuthCookie:     args.GlobalConf.CNCAuth.SessionCookieName,
			AuthTokenEntry:    cnc.AuthTokenEntry,
			ServicePath:       fmt.Sprintf("/service/%d/frodo", args.SID),
			ServiceKey:        fmt.Sprintf("%d/frodo", args.SID),
	...
	args.APIRoutes.Any(
		fmt.Sprintf("/service/%d/frodo/*path", args.SID),
	...
	log.Info().Int("args.SID", args.SID).Msg("Proxy for Frodo enabled")
	return nil
}
